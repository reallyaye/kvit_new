...
class DatabaseSessionStore(BaseSessionStore):
    ...
    def get_session_expiry(self, token: str) -> Optional[float]:
        """
        Проверяет сессию:
        1. Fast Path: L1 Cache (если не истёк L1 TTL и не истёк expires_at).
        2. Slow Path: Запрос к L2 БД при Cache-Miss или окончании L1 TTL.
        3. При сбое L2 БД: применение failure_policy (FAIL_CLOSED / FAIL_OPEN_L1 / STRICT).
        """
