...
def render_appeal_status_page(is_admin=False):
    ...
    (() => {{
      ...
      const renderSaved=()=>{{const items=readSaved();savedList.textContent='';savedBox.hidden=!items.length;savedEmpty.hidden=Boolean(items.length);if(!items.length)return;items.forEach(item=>{{const link=document.createElement('a'),number=document.createElement('span'),meta=document.createElement('div'),date=document.createElement('span'),params=new URLSearchParams({{number:item.number,code:item.code}});link.className='saved-appeal-link';link.href=`/appeals/status#${{params.toString()}}`;link.addEventListener('click',(event)=>{{event.preventDefault();loadAppeal(item.number,item.code);}});number.textContent=item.number;meta.className='saved-appeal-meta';if(item.status){{const badge=document.createElement('span');badge.className=`saved-appeal-badge st-${{item.status.toLowerCase()}}`;badge.textContent=statuses[item.status]||item.status;meta.append(badge);}}date.textContent=item.savedAt?new Intl.DateTimeFormat(document.documentElement.lang==='kk'?'kk-KZ':'ru-RU').format(new Date(item.savedAt)):'Открыть';meta.append(date);link.append(number,meta);savedList.append(link);}});}};
      ...
      form.addEventListener('submit',async(event)=>{{event.preventDefault();error.style.display='none';result.className='appeal-status-result';result.textContent='';button.disabled=true;
        try{{const response=await fetch(form.action,{{method:'POST',headers:{{'Accept':'application/json'}},body:new URLSearchParams(new FormData(form))}});const data=await response.json();if(!response.ok)throw new Error(data.message||'Не удалось проверить обращение.');
          const currentCode=form.elements.credential.value;const current={{number:data.registration_number,code:currentCode,savedAt:Date.now(),status:data.status}};writeSaved([current,...readSaved().filter(item=>item.number!==data.registration_number)]);renderSaved();
          ...
          const answer=document.createElement('div');if(data.response_text){{const title=document.createElement('h2');title.textContent='Ответ на ваше обращение';answer.className='appeal-public-answer';answer.textContent=data.response_text;result.append(title,answer);}}else if(data.status==='CLOSED'){{answer.className='appeal-awaiting appeal-closed';answer.textContent='Обращение рассмотрено и закрыто.';result.append(answer);}}else if(data.status==='REJECTED'){{answer.className='appeal-awaiting appeal-rejected';answer.textContent='Обращение отклонено.';result.append(answer);}}else if(data.status==='ANSWERED'){{answer.className='appeal-awaiting appeal-answered';answer.textContent='Обращение обработано.';result.append(answer);}}else{{answer.className='appeal-awaiting';answer.textContent='Обращение находится на рассмотрении. Ответ ещё не подготовлен, зайдите позже.';result.append(answer);}}result.className='appeal-status-result show';result.scrollIntoView({{block:'nearest'}});
