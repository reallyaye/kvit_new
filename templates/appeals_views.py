...
def render_appeal_status_page(is_admin=False):
    ...
    <section class="appeal-status-page">
      <div class="appeal-status-card">
        ...
        <h2 class="appeal-manual-title">Найти обращение вручную</h2>
    ...
    (() => {{
      ...
      const renderSaved=()=>{{const items=readSaved();savedList.textContent='';savedBox.hidden=!items.length;if(!items.length)return;items.forEach(item=>{{const link=document.createElement('a'),number=document.createElement('span'),date=document.createElement('span'),params=new URLSearchParams({{number:item.number,code:item.code}});link.className='saved-appeal-link';link.href=`/appeals/status#${{params.toString()}}`;number.textContent=item.number;date.textContent=item.savedAt?new Intl.DateTimeFormat(document.documentElement.lang==='kk'?'kk-KZ':'ru-RU').format(new Date(item.savedAt)):'Открыть';link.append(number,date);savedList.append(link);}});}};
      ...
      if(savedNumber&&savedCode){{const current={{number:savedNumber,code:savedCode,savedAt:Date.now()}};writeSaved([current,...readSaved().filter(item=>item.number!==savedNumber)]);form.elements.registration_number.value=savedNumber;form.elements.credential.value=savedCode;form.requestSubmit();}}
      renderSaved();
    }})();
    </script>'''
